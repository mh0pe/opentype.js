var opentype = (() => {
  ...
  function makeCmapTable(glyphs) {
    ...
    for (let i2 = 0; i2 < mergedSegments.length; i2++)
      if (mergedSegments[i2].start <= 65535)
        bmpSegmentCount++;
    ...
    for (i = 0; i < segCount; i += 1) {
      ...
      if (segment.end <= 65535 && segment.start <= 65535 && !cmap4Overflows || cmap4Overflows && segment.end === 65535 && segment.start === 65535) {
  ...
  function computeCheckSum(bytes) {
    ...
    for (let i = 0; i < padded; i += 4) {
      sum += (((bytes[i] || 0) & 255) << 24 | ((bytes[i + 1] || 0) & 255) << 16 | ((bytes[i + 2] || 0) & 255) << 8 | (bytes[i + 3] || 0) & 255) >>> 0;
    }
    return sum >>> 0;
  }
  ...
  function fontToSfntTable(font) {
    ...
    for (let i = 0; i < tableFields.length; i += 1) {
      if (tableFields[i].name === "head table") {
        tableFields[i].value.checkSumAdjustment = 2981146554 - checkSum >>> 0;
  ...
  function applyChainingLookupRecords(contextParams, rule) {
    ...
    for (let i = 0; i < rule.lookupRecords.length; i++) {
      ...
      for (let s = 0; s < lookupTable.subtables.length; s++) {
        ...
        if (substitution !== null && substitution !== void 0) {
  ...
  function chainingSubstitutionFormat2(contextParams, subtable) {
    ...
    for (let i = 0; i < chainClassSet.length; i++) {
      ...
      const substitutions = rule.lookupRecords.length ? applyChainingLookupRecords.call(this, contextParams, rule) : [];
  ...
  function chainingSubstitutionFormat22(action, tokens, index) {
  ...
  var SUBSTITUTIONS = {
    ...
    62: chainingSubstitutionFormat22,
